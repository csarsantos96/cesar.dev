 ...
 * Campos em DO_NOT_TRANSLATE ficam identicos (nomes proprios, empresas,
 * tecnologias, datas, URLs) - como no portfolio original.
 *
...
// Modelo Gemini. Troque aqui se usar outro no commandlinux (ex: gemini-3.5-flash).
const MODEL = 'gemini-2.5-flash';
const SOURCE_LANG = 'Portuguese (Brazil)';
const TARGET_LANG = 'English';

// Chaves cujo valor NAO e traduzido.
...
function collect(node, bag, key = null) {
  if (typeof node === 'string') {
    if (key && DO_NOT_TRANSLATE.has(key)) return node;
    bag.push(node);
    return { __ref: bag.length - 1 };
  }
  if (Array.isArray(node)) return node.map((v) => collect(v, bag, key));
  if (node && typeof node === 'object') {
    const out = {};
    for (const [k, v] of Object.entries(node)) out[k] = collect(v, bag, k);
    return out;
  }
  return node;
}
...
const translated = await translateBatch(bag);
const result = rebuild(skeleton, translated);
await writeFile(OUT, JSON.stringify(result, null, 2) + '\n');
console.log(`OK: ${bag.length} strings traduzidas -> ${OUT}`);
